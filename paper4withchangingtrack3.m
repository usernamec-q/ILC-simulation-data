...
W11 = [0.7, 0]; W12 = [0.3, 0]; L11 = [0.3 0]; L12 = [0.8 -0.2];
W21 = [0, 0.7]; W22 = [0, 0.3]; L21 = [-0.6 -0.05]; L22 = [0.6 0.15];
    ...
    if tk<50
        for t=2:tk
            ...
            if mod(k, 2) == 0  % 偶数迭代
                ...
                A21 = 1.22; A22=0; B21 = 0; B22 =5;
                C11 = 0.15*exp(-t/100); C12=0;
                C21 = 0; C22 = 0.4*exp(-t/100);
            else  % 奇数迭代
                % A11 = 0; A12=1.3; A21 =1 ;  A22=0;
                % B11 = exp(-(t-1)/100); B12 = 0;  B21 =0;B22 =5.2*cos(t) ;
                A11 = 0; A12=1.22; B11 = 0.95*exp(-(t-1)/95); B12 = 0;
                A21 = 1.2; A22=0; B21 = 0; B22 =4.9;
                C11 = 0.18*exp(-t/100); C12=0;
                C21 = 0; C22 = 0.42*exp(-t/100);
            end

            %第k次迭代的x(t)
            %x1(k,t)=[0.4 0]*[x1(k,t-1);x2(k,t-1)]+[0.06 0.8]*[u1(k,t-1);u2(k,t-1)];
            %x2(k,t)=[0.65 0.2]*[x1(k,t-1);x2(k,t-1)]+[5.6 0.5]*[u1(k,t-1);u2(k,t-1)];
            x3(k,t)=A11*sin(x3(k,t-1))+A12*sin(x4(k,t-1))+cos(t-1)+[B11 B12]*[u3(k,t-1);u4(k,t-1)];
            x4(k,t)=A21*cos(x3(k,t-1))+A22*cos(x4(k,t-1))+[B21 B22]*[u3(k,t-1);u4(k,t-1)];

            %输出轨迹：第k次迭代的y(t)
            y1(k,t)=[C11 C12]*[x3(k,t);x4(k,t)];
            y2(k,t)=[C21 C22]*[x3(k,t);x4(k,t)];
