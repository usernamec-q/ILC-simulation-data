...
for k = 1:m
    ...
    % =================核心修改部分=================
    % 引入随迭代次数增加而衰减的因子 (k越大，衰减因子越趋近于0)
    % 初始状态有偏移（原奇数次特性），最终收敛至常数参数（原偶数次特性，确保ILC收敛）
    ...
    B11 = exp(-(t-1)/100 * (1 - 0.2 * decay_ratio)); % 指数系数从 0.8 趋近于 1.0
    B12 = 0; 
    B21 = 0;
    B22 = 5 + (5.2 * cos(t) - 5) * decay_ratio; % 从含有非线性波动 5.2*cos(t) 趋近于定值 5
    
    C1 = 0.15 * exp(-t/100); 
    C2 = (0.4 + 0.12 * decay_ratio) * exp(-t/100);   % 从 0.52 趋近于定值 0.4
    % ==============================================
    
    y1(k,1) = C1*x1(k,1) + v1(k, 1);  %实际迭代输出1
    y2(k,1) = C2*x2(k,1) + v2(k, 1);  %实际迭代输出2
    ...
    if tk < 103  
        for t = 2:tk
            x1(k,t) =A11*x1(k,t-1) +A12*sin(x2(k,t-1))+cos(t-1)+B11*u1(k,t-1)+B12*u2(k,t-1)+w1(k,t);
            x2(k,t) =A21*cos(x1(k,t-1))+A22*x2(k,t-1)+B21*u1(k,t-1)+B22*u2(k,t-1)+ w2(k,t);
            y1(k,t) = C1 * x1(k,t) + v1(k,t);
            y2(k,t) = C1 * x2(k,t) + v2(k,t);
            % 误差计算
            e1(k, t) = y_d1(1, t) - y1(k, t);
            e2(k, t) = y_d2(1, t) - y2(k, t);
            e1(k, t+1) = y_d1(1, t+1) - C1*(A11*x1(k,t-1) +A12*sin(x2(k,t-1))+cos(t-1)+B11*u1(k,t-1)+B12*u2(k,t-1));
            e2(k, t+1) = y_d2(1, t+1) - C2*(A21*cos(x1(k,t-1))+A22*x2(k,t-1)+B21*u1(k,t-1)+B22*u2(k,t-1));
            ...
            if k == 1
                ...
                u2(k,t) = 0;
            elseif k==2
                u1(k, t) = [1,0]*[u1(k-1, t); u2(k-1, t)] + [0.3,0] * [e1(k-1, t+1); e2(k-1, t+1)];
                u2(k, t) = [1,0]*[u1(k-1, t); u2(k-1, t)] + [0.6,-0.05] * [e1(k-1, t+1); e2(k-1, t+1)];
            else
        ...
        for t=2:103
            ...
            if k == 1
                ...
                u2(k,t) = 0;
            elseif k==2
                u1(k, t) = [1,1]*[u1(k-1, t); u2(k-1, t)] + L11 * [e1(k-1, t+1); e2(k-1, t+1)];
                u2(k, t) = [1,1]*[u1(k-1, t); u2(k-1, t)] + L21 * [e1(k-1, t+1); e2(k-1, t+1)];
            else
                u1(k, t) = W11*[u1(k-1, t); u2(k-1, t)] + W12*[u1(k-2, t); u2(k-2, t)] + L11 * [e1(k-1, t+1); e2(k-1, t+1)] + L12 * [e1(k-2, t+1); e2(k-2, t+1)];
                u2(k, t) = W21*[u1(k-1, t); u2(k-1, t)] + W22*[u1(k-2, t); u2(k-2, t)] + L21 * [e1(k-1, t+1); e2(k-1, t+1)] + L22 * [e1(k-2, t+1); e2(k-2, t+1)];
            end
